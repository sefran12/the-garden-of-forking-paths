...
the_garden = """You are Destiny, master of the Garden of Forking Paths. You embody your voice and give the player the choice between two paths in the garden, which encompass actions and decisions, and always only between two actions. These must be different between them, and should lead to different paths. Each decision you give should be just a small step in the journey, not the whole journey, so good decisions are simple actions, simple scenarios. Remember, the Garden of Forking Paths goes on forever, until Death, so after any decision, there will come many other decisions.
...
def extract_dict_content(response_text):
    # Find JSON content enclosed in backticks or triple backticks
...
async def get_initial_choices():
    client = AsyncClient()
    messages = [
        {"role": "system", "content": the_garden},
        {"role": "user", "content": traveler}
    ]
    stream = await client.chat(model=LLM_MODEL, messages=messages, stream=True)
    response_text = ''
    async for part in stream:
        content = part['message']['content']
        # Collect the response text but do not print it
        response_text += content
    # Parse the JSON content
    return extract_dict_content(response_text)

# Function to get subsequent choices from the API
async def get_choices(history):
    the_next_island = f"""You are Destiny, master of the Garden of Forking Paths. You embody your voice and give the player the choice between two paths in the garden, which encompass actions and decisions, and always only between two actions. These must be different between them, and should lead to different paths. Each decision you give should be just a small step in the journey, not the whole journey, so good decisions are simple actions, simple scenarios. Remember, the Garden of Forking Paths goes on forever, until Death, so after any decision, there will come many other decisions.
...
async def get_vision(traveler, history, astral_card):
    the_vision = f"""You are Destiny, master of the Garden of Forking Paths. And now, you are Heart, also called Maya in the Vedic texts, which reveals the veil placed upon all things. The traveler has chosen a path, and has trodden in it. What has happened now that he has made a decision?

Express what the traveler sees in the island succinctly, in 200 words. Again, the vision is always just a scene, never the full journey, even if a simulacrum. It should show, never tell, and it should be an instant in time, a particular moment.

This is the traveler: {traveler}

This is their astral card, unknown to them, which affects their fortune. This knowledge is secret to you, and cannot be expressed to the traveler. It just influences their travel: {astral_card}

And this is the history of their travel through the Garden of Forking Paths: {history} """
    ...
    async for part in stream:
        content = part['message']['content']
        # You can choose to print the vision as it streams in
        print(content, end='', flush=True)
        response_text += content
    print()  # For a new line after streaming
...
async def play_game():
    ...
    # Function to flush history
    def flush_history(history):
        flushed_count = 0
        while len(history) > 32000:
            # Remove the oldest island entry
            island_end = history.find("\n# Island", 1)
            if island_end == -1:
                # If no more islands, truncate from the start
                history = history[-32000:]
                flushed_count += 1
            else:
                history = history[island_end:]
                flushed_count += 1
        
        if flushed_count > 0:
            print(f"{flushed_count} memories were flushed from history.")
        
        return history

    # Initial choice
    try:
        choices_dict = await get_initial_choices()
    except Exception as e:
        print_formatted(f"Error getting initial choices: {e}", Fore.RED)
        return
    ...
    vision = await get_vision(traveler, history, astral_card)

    # Update history with the initial vision
    history += f"Maya, the Heart, showed him this vision: {vision}\n"
    history = flush_history(history)  # Flush history
    ...
    for turn in range(1, 50):  # Assuming we want to play for 50 turns in total
        try:
            choices_dict = await get_choices(history)
        except Exception as e:
            print_formatted(f"Error getting choices: {e}", Fore.RED)
            continue
